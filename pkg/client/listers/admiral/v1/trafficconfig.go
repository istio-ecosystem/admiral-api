...
type TrafficConfigLister interface {
	...
	List(selector labels.Selector) (ret []*v1.TrafficConfig, err error)
...
// List lists all TrafficConfigs in the indexer.
func (s *trafficConfigLister) List(selector labels.Selector) (ret []*v1.TrafficConfig, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.TrafficConfig))
	})
	return ret, err
}
...
type TrafficConfigNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.TrafficConfig, error)
...
// List lists all TrafficConfigs in the indexer for a given namespace.
func (s trafficConfigNamespaceLister) List(selector labels.Selector) (ret []*v1.TrafficConfig, err error) {
...
// Get retrieves the TrafficConfig from the indexer for a given namespace and name.
func (s trafficConfigNamespaceLister) Get(name string) (*v1.TrafficConfig, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("trafficconfig"), name)
	}
	return obj.(*v1.TrafficConfig), nil
}
